...
const Reports = () => {
  ...
  // ---------- Syllabus export (admin only) ----------
  const handleExportSyllabus = async () => {
    if (role !== "admin") {
      alert("Only admins can export syllabus.");
      return;
    }
    // month/year selection: if period is used we won't export (require explicit month/year)
    if (!!period) {
    ...
    try {
      const params = { month: String(Number(month)), year: String(year) };
      ...
      a.href = url;
      a.download = `syllabus_${year}_${String(Number(month)).padStart(2, "0")}.xlsx`;
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {/* Filters */}
          <div className="flex flex-wrap gap-3">
            ...
            {/* Admin-only: Syllabus export */}
            {role === "admin" && (
              <div className="flex items-end gap-3">
                <button
                  onClick={handleExportSyllabus}
                  className="px-4 py-2 bg-green-600 text-white rounded-xl shadow hover:brightness-105"
                >
                  Export Syllabus (Excel)
                </button>
              </div>
            )}
          </div>
